...
const LEAGUES: { code: string; label: string; labelZh: string }[] = [
  { code: "eng.1", label: "Premier League", labelZh: "英格兰超级联赛" },
  { code: "esp.1", label: "La Liga", labelZh: "西甲联赛" },
  { code: "ger.1", label: "Bundesliga", labelZh: "德甲联赛" },
  { code: "ita.1", label: "Serie A", labelZh: "意甲联赛" },
  { code: "fra.1", label: "Ligue 1", labelZh: "法甲联赛" },
  { code: "usa.1", label: "MLS", labelZh: "美国职业大联盟" },
  { code: "mex.1", label: "Liga MX", labelZh: "墨西哥超级联赛" },
  { code: "bra.1", label: "Brasileirão Série A", labelZh: "巴西甲级联赛" },
  { code: "arg.1", label: "Liga Profesional (Argentina)", labelZh: "阿根廷甲级联赛" },
  { code: "chn.1", label: "Chinese Super League", labelZh: "中国足球超级联赛" },
];

/** Cap per league so one busy league (e.g. a full MLS matchday) doesn't crowd out the rest. */
...
async function fetchLeagueMatches(league: {
  ...
  try {
    const res = await fetch(
      `https://site.api.espn.com/apis/site/v2/sports/soccer/${league.code}/scoreboard?dates=${trailingDateRange()}`,
      {
    ...
    for (const event of data.events ?? []) {
      ...
      // Skip fixtures missing a name or crest — we show club logos, not flags,
      // so a match without both crests can't render consistently.
      ...
      matches.push({
        id: event.id
          ? `${league.code}-${event.id}`
          : `${league.code}-${home.displayName}-${away.displayName}-${event.date}`,
        competition: league.label,
        competitionZh: league.labelZh,
        homeTeam: home.displayName,
        homeTeamZh: clubNameCn(league.code, home.displayName),
        homeLogo: home.logo,
        awayTeam: away.displayName,
        awayTeamZh: clubNameCn(league.code, away.displayName),
        awayLogo: away.logo,
    ...
    // league (live/recent/soonest) rather than an arbitrary chronological
    // prefix, since the window can span several days.
    matches.sort(compareMatches);
    return matches.slice(0, MAX_PER_LEAGUE);
  } catch {
    // Network error or timeout — this league contributes nothing today.
    return [];
  }
}

/** Fetches today's matches across all tracked leagues, live-first then soonest, most-recent-finished last. */
